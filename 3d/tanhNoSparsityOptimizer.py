...
proteomics_features = data_proteomics_transposed.iloc[1:64, 8:40]
...
learning_rates = [.005]#np.arange(0.001, 0.02, 0.001)
l2_regs = np.arange(0.01, 0.10, 0.01)
dropout_rates = [.75,.5,.4,.3,0.2, 0.1, 0]
...
for lr in learning_rates:
    for l2_reg in l2_regs:
        for dropout_rate in dropout_rates:
            db_index = []
            for i in range(10):
                db_index_value = build_autoencoder(lr, l2_reg, dropout_rate)
                db_index.append(db_index_value)

                if db_index_value == 0:
                    db_index_value = 100
            avg_db_index = np.mean(db_index)
            print(f"Parameters: lr={lr}, l2={l2_reg}, dropout={dropout_rate} --> Davies-Bouldin Index: {avg_db_index}")

            if avg_db_index < best_db_index and avg_db_index != 0:
                best_db_index = avg_db_index
